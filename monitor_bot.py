...
# -*- coding: utf-8 -*-
"""
VWAP Reversal Monitor Bot – Production Edition
- SL cooldown, daily loss limit, continuous TP/SL monitoring
- Full‑day VWAP, weekly TP/SL optimization, daily pair selection
- Clean, readable heartbeat
"""
...
# Current trading set – updated daily at 12:00 UTC
PAIRS = ALL_PAIRS[:21]  # initial fallback, will be overwritten on first daily run
...
DAILY_LOSS_LIMIT = 15.0 # USD

STARTING_BALANCE = 100.0

# Initial TP/SL (will be overwritten after first optimization)
OPTIMIZED_TP_SL = {}
...
def fetch_live_data(symbol):
    """Last 5 hours + full today's data for VWAP accuracy."""
    now = datetime.now(timezone.utc)
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    end_ms = int(now.timestamp() * 1000)
    start_ms = int((now - timedelta(hours=5)).timestamp() * 1000)
    # Merge: today's data + last 5 hours
    ...
    df_combined = pd.concat([df_today[~df_today.index.isin(common)], df_recent]).sort_index()
    return df_combined
...
def load_historical_data(symbol, days):
    ...
    if cp.exists():
        try:
            ...
            pass
    # Fallback: download if not cached (unlikely on Render, but safe)
    now = datetime.now(timezone.utc)
...
def check_signal(df, pair_cooldown_until):
    ...
    if pair_cooldown_until and now_ts < pair_cooldown_until:
        return 0, {**info, "fail": "cooldown"}
...
signal_counter = 0
open_trades = {}
cooldowns = {}          # pair -> timestamp until cooldown
daily_pnl = 0.0         # reset at 00:00
...
async def optimize_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Optimizing TP/SL and pair selection...")
    # Run daily tasks in background
    asyncio.create_task(daily_pair_selection_and_optimize())
    await update.message.reply_text("Optimization started. Results will be sent shortly.")

# ═════════════════ MAIN BOT LOOP ═════════════════
async def continuous_tp_monitor():
    """Check open trades every 30 seconds for TP/SL hits."""
    await asyncio.sleep(5)  # give the main loop a head start
    while True:
        now = datetime.now(timezone.utc)
        closed_ids = []
        for trade_id, t in list(open_trades.items()):
            sym = t["pair"]
            # fetch only the very last candle
            df = fetch_klines(sym, int((now - timedelta(minutes=2)).timestamp()*1000),
            ...
            if hit:
                ...
                trade_log.append({
                    ...
                    "outcome": hit, "pnl": pnl, "timestamp": now
                })
                ...
                daily_pnl += pnl
                if hit == "SL":
                    cooldowns[sym] = now + timedelta(minutes=SL_COOLDOWN)
...
async def daily_pair_selection_and_optimize():
    """Download 30-day data for all pairs, run OOS backtest, select top 20, optimize TP/SL."""
    global PAIRS, OPTIMIZED_TP_SL
    await application.bot.send_message(chat_id=CHAT_ID, text="⏳ Daily pair selection & TP/SL optimization starting...")
    now = datetime.now(timezone.utc)
    end_ms = int(now.timestamp() * 1000)
    ...
    for sym in ALL_PAIRS:
        ...
        # Single window: last 15d train, previous 5d test
    ...
    if not results:
        await application.bot.send_message(chat_id=CHAT_ID, text="❌ Not enough data for daily optimization.")
        return

    # Rank by OOS PnL, pick top 20 with positive PnL
    sorted_pairs = sorted(results.items(), key=lambda x: x[1]['oos_pnl'], reverse=True)
    top_pairs = [sym for sym, data in sorted_pairs if data['oos_pnl'] > 0][:20]
    if len(top_pairs) < 5:
        # fallback to top 20 by OOS PnL even if negative? No, keep at least 5 best
        top_pairs = [sym for sym, data in sorted_pairs][:20]

    PAIRS = top_pairs
    OPTIMIZED_TP_SL = {sym: (results[sym]['tp'], results[sym]['sl']) for sym in top_pairs}

    summary = f"✅ Daily optimization done.\n🟢 New pair list ({len(PAIRS)} pairs): {', '.join(PAIRS[:10])}..."
...
async def monitor():
    ...
    # Start continuous TP monitor
    asyncio.create_task(continuous_tp_monitor())

    bot = application.bot
    await bot.send_message(chat_id=CHAT_ID, text="🟢 Production bot started. Features: cooldown, daily loss limit, continuous TP, full VWAP, daily pair selection.")

    # Run initial optimization if not done
    ...
    while True:
        ...
        if daily_pnl <= -DAILY_LOSS_LIMIT:
            # Send heartbeat but no new signals
            await bot.send_message(chat_id=CHAT_ID, text="⚠️ Daily loss limit reached. No new signals until midnight.")
            await asyncio.sleep(SCAN_INTERVAL_MINUTES * 60)
            continue

        # Clear expired cooldowns
        expired = [pair for pair, until in cooldowns.items() if now >= until]
        for pair in expired:
            del cooldowns[pair]
        ...
        for sym in PAIRS:
            ...
            if live.empty:
                no_trade += 1
                continue
            df_hist = load_historical_data(sym, LOOKBACK_DAYS)
            if not df_hist.empty:
                common = live.index.intersection(df_hist.index)
                df_hist = df_hist[~df_hist.index.isin(common)]
                df = pd.concat([df_hist, live]).sort_index()
            else:
                df = live
            df = add_indicators(df)
            cooldown_until = cooldowns.get(sym)
            signal, info = check_signal(df, cooldown_until)
            ...
            # Build line for high-score pairs
            if score >= 3:
            ...
            # Generate trade signal if all checks passed
            if signal != 0 and daily_pnl > -DAILY_LOSS_LIMIT:
                signal_counter += 1
                side = signal
                tp_price = price * (1 + tp_sl[0]) if side == 1 else price * (1 - tp_sl[0])
                sl_price = price * (1 - tp_sl[1]) if side == 1 else price * (1 + tp_sl[1])
                ...
                alert_text = (
                    ...
                    f"🧠 Reason:\n{reason}\n"
                    f"━━━━━━━━━━━━━━━━━\n"
                    f"✅ TP: ${tp_price:.6f} (+{tp_sl[0]*100:.2f}%)\n"
                    f"❌ SL: ${sl_price:.6f} (-{tp_sl[1]*100:.2f}%)"
                )
                keyboard = InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ TP Hit", callback_data=f"TP|{sym}|{side}|{price}|{tp_price}|{sl_price}|{signal_counter}"),
                     InlineKeyboardButton("❌ SL Hit", callback_data=f"SL|{sym}|{side}|{price}|{tp_price}|{sl_price}|{signal_counter}")]
                ])
                sent_msg = await bot.send_message(chat_id=CHAT_ID, text=alert_text,
                                                   reply_markup=keyboard, parse_mode='Markdown')
                signal_log.append({
                    "id": signal_counter, "pair": sym, "side": side, "entry": price,
                    "tp": tp_price, "sl": sl_price, "timestamp": now,
                    "outcome": None, "pnl": None
                })
                open_trades[f"{sym}_{int(time.time())}"] = {
                    "pair": sym, "side": side, "entry": price, "tp": tp_price, "sl": sl_price,
                    "alert_msg": sent_msg, "chat_id": CHAT_ID, "signal_id": signal_counter
                }
        ...
        # Daily optimization at 12:00 UTC
        if now.hour == 12 and now.minute == 0:
            asyncio.create_task(daily_pair_selection_and_optimize())

        # Sleep until next scan
        await asyncio.sleep(SCAN_INTERVAL_MINUTES * 60 - (datetime.now(timezone.utc) - now).seconds)
